...
def mclust_R(adata, num_cluster, modelNames='EEE', used_obsm='emb_pca', random_seed=2024):
    ...
    for i in range(adata.obsm['emb_pca'].shape[1]):
        sns.kdeplot(adata.obsm['emb_pca'][:, i], label=f"PCA Component {i+1}", alpha=0.6)
...
def find_overlap_genes(adata, adata_sc):
    has_highly_variable_st = 'highly_variable' in adata.var.keys()
    has_highly_variable_sc = 'highly_variable' in adata_sc.var.keys()
    
    if has_highly_variable_st:
        adata = adata[:, adata.var['highly_variable']]
    
    if has_highly_variable_sc:
        adata_sc = adata_sc[:, adata_sc.var['highly_variable']]

    genes = list(set(adata.var_names) & set(adata_sc.var_names))
    genes.sort()  
    print(f'find overlap genes:{len(genes)} ')
