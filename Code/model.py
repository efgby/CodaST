...
class Encoder(Module):
     def __init__(self, in_features, out_features, graph_neigh, dropout=0.0, act=F.relu, random_seed=41):
        ...
        self.decoder = buildNetwork([self.out_features]+[num_hidden], input_dim=self.in_features,activation='elu', dropout=0.)
...
class Encoder_sc(torch.nn.Module): 
    def __init__(self, input_dim=2000, output_dim=128):
        ...
        self.decoder = torch.nn.Sequential(
            torch.nn.Linear(self.output_dim, 512),
            torch.nn.BatchNorm1d(512),
            torch.nn.ReLU(),
...
class Encoder_map(torch.nn.Module):
    ...
    def forward(self):
        return self.M  # shape: (n_spot, n_cell)
